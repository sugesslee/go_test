 ...
 * </pre>
 * @author redli
 * @version 1.0.0 2019/11/2 4:06 PM
 * @date 2019/11/2 4:06 PM
 * @since 1.0.0
 */
type GRPCServer struct {
	// Listen address for the server specified as hostname:port
	address string
	// Listen for handling network requests
...
func NewGRPCServerFromListener(listener net.Listener, serverConfig ServerConfig) (*GRPCServer, error) {
	...
	if len(serverConfig.StreamInterceptors) > 0 {
		serverOpts = append(
			serverOpts,
			grpc.StreamInterceptor(grpcmiddleware.ChainStreamServer(serverConfig.StreamInterceptors...)),
		)
	}
	if len(serverConfig.UnaryInterceptors) > 0 {
		serverOpts = append(
			serverOpts,
			grpc.UnaryInterceptor(grpcmiddleware.ChainUnaryServer(serverConfig.UnaryInterceptors...)),
		)
	}
